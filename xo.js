...
xover.server = new Proxy({}, {
    get: function (self, key) {
        ...
        let handler = (async function (payload, ...args) {
            ...
            let server_url = new xover.URL(xover.manifest.server[key], undefined, { payload, ...settings })
            let url = this instanceof URL && this || this instanceof Document && this.url || server_url;
            if (!(server_url.origin == url.origin && server_url.pathname == url.pathname)) {
                url = server_url
            }
            url.payload = payload;
            ...
            window.top.dispatchEvent(new xover.listener.Event(`beforeFetch`, { url, request, href: url.href, tag: `server:${key}` }, url));
            try {
    ...
    Object.defineProperty(this, `fetch`, {
        value: async function (...args) {
            ...
            while (!response && sources.length) {
                ...
                if (location.origin == source_url.origin) { //xover.URL automatically supports ?searchParams to come after or before hash;
                    let [hash, searchParams = ''] = source_url.hash.split("?");
                    source_url.hash = hash;
                    source_url.search = searchParams || source_url.search;
                }
                this.url = source_url;

                let settings = xover.json.merge({}, xover.manifest.getSettings(source_url), self.settings, this.settings);
                        ...
                        promises.push(new Promise(async (resolve, reject) => {
                            try {
                                if (endpoint.replace(/^server:/, '') in xover.server) {
                                    response = await xover.server[endpoint.replace(/^server:/, '')].call(this);
...
Object.defineProperty(URL.prototype, 'href', {
    get: function (...args) {
        let href = URL.href.get.apply(this, args);
    ...
    this.modernizing = this.modernizing || xover.delay(0).then(async () => {
            ...
            if (targetWindow.document.implementation.hasFeature("XPath", "3.0")) {
                ...
                if (!Node.prototype.hasOwnProperty('observe')) {
                    Node.prototype.observe = function (config = { characterData: true, attributes: true, childList: true, subtree: true, attributeOldValue: true, characterDataOldValue: true }) {
                        ...
                        xover.xml.observer.observe(self, config);
        ...
        `), 'text/xml');
                ...
                Element.prototype.seed = function (reseed) {
                    ...
                    //observer && observer.disconnect(0)
                    this.selectNodes(`descendant-or-self::*[not(@xo:id!="")]`).forEach(node => Element.setAttributeNS.call(node, xover.spaces["xo"], 'xo:id', (function (node) { return `${node.nodeName}_${xover.cryptography.generateUUID()}`.replace(/[:-]/g, '_') })(node)));
