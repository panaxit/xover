...
xover.server = new Proxy({}, {
    get: function (self, key) {
        ...
        let handler = (async function (...args) {
            ...
            window.top.dispatchEvent(new xover.listener.Event(`response`, { response_value: return_value, response, payload, request, tag: `#server:${key}` }, response));
        ...
        if (!history.state['seed']) {
            ...
            prev.unshift({
                ...
                , ref: {
                    ...
                    , attrib: ref_node instanceof Attr && ref_node.name || null
                }
            });
...
xover.listener.on('success::xo:message[.!=""]', function ({ target, attribute: key }) {
    this.render()
});
                    ...
                    value: async function (stylesheets) {
                        ...
                        for (let stylesheet of stylesheets.filter(stylesheet => stylesheet.role != "init" && stylesheet.role != "binding")) {
                            ...
                            if (!target) {
                                ...
                                if (dom.selectNodes) { //(dom instanceof XMLDocument) {
                            ...
                            dom.querySelectorAll('[xo-stylesheet]:not([xo-section]').forEach(el => data.render(
