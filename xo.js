...
Object.defineProperty(xover.listener, 'dispatchEvent', {
    value: async function (event, axis = {}) {
        ...
        if (axis instanceof Attr) {
            ...
            event.detail["response"] = event.detail["response"] || axis;
            event.detail["resquest"] = event.detail["request"] || axis.request;
            event.detail["body"] = event.detail["body"] || axis.body;
            node = event.detail["return_value"] || axis.document;
...
xover.fetch.xml = async function (url, settings = { rejectCodes: 500 }, on_success) {
    ...
    if (xover.session.debug) {
        return_value.$$(`//xsl:template[not(contains(@mode,'-attribute'))]/*[not(contains(string(@mode),'-attribute'))][not(self::xsl:param or self::xsl:text or self::xsl:value-of or self::xsl:choose or self::xsl:if or self::xsl:attribute or self::xsl:variable or ancestor::xsl:element)]|//xsl:template//xsl:*//html:option`).forEach(el => {
...
xover.listener.on(["contextmenu", "focusin"], function (event) {
    ...
    xover.delay(40).then(() => {
        ...
        let target_section = target.section;
        if (target_section instanceof xover.Section && (event.type == 'contextmenu' || target instanceof HTMLInputElement || target instanceof HTMLSelectElement || target instanceof HTMLTextAreaElement)) {
    ...
    with (targetWindow) {
        ...
        `), 'text/xml');
            ...
            Attr.prototype.toggle = function (value, else_value = '') {
                value = typeof value === 'function' && value.call(this, this.value) || value && value.constructor === {}.constructor && JSON.stringify(value) || value != null && String(value) || value;
                //if (this.value != value) {
                this.parentNode.section && this.parentNode.section.render();
                //}
                if (this.value == value) {
                ...
                Object.defineProperty(XMLDocument.prototype, 'render', {
                    value: async function (stylesheets) {
                        ...
                        for (let stylesheet of stylesheets.filter(stylesheet => stylesheet.role != "init" && stylesheet.role != "binding")) {
                            ...
                            target.flatMap(el => [...el.querySelectorAll('input[xo-attribute],select[xo-attribute],textarea[xo-attribute],input[type="file"]')]).map(el => el.addEventListener('change', async function () {
